...
func (p *itmRawPrinter) ObservePacket(indexSOP uint64, pkt fmt.Stringer, rawData []byte) {
	if len(rawData) == 0 {
		return
	}
...
func TestTraceItmGoldens(t *testing.T) {
	...
	var traceIndex Index = 0

	cfg := &itmConfig{RegTCR: 0}
	decoder, err := itmNewDecoder(cfg)
	if err != nil {
