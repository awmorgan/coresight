...
const (
	stateProcHdr processState = iota
	stateProcData
	stateSendPkt
	stateSendUnsynced
)

type packetHandler func(*Decoder, byte) error
...
type parseContext struct {
	...
	currHandler  packetHandler
	currPacket   Packet
	packetIndex  trace.Index
	...
	updateUnsyncPacketIndex trace.Index
...
func (d *Decoder) processData(index trace.Index, data []byte) (uint32, error) {
	...
	for d.ctx.Reader.Len() > 0 || d.ctx.processState == stateSendPkt {
		...
		case stateProcHdr:
			...
			if d.ctx.isSync {
				...
				d.ctx.currHandler = (*Decoder).iNotSync
			}
		...
		case stateProcData:
			...
			for d.ctx.Reader.Len() > 0 && d.ctx.processState == stateProcData {
...
func (d *Decoder) outputUnsyncedRawPacket() {
...
func (d *Decoder) initPacketState() {
	d.ctx.raw = d.ctx.raw[:0]
	d.ctx.currPacket.InitNextPacket()
	d.ctx.updateUnsyncPacketIndex = 0
}

func (d *Decoder) initProcessorState() {
	d.initPacketState()
	d.ctx.processState = stateProcHdr
	d.ctx.currHandler = (*Decoder).iNotSync
	d.ctx.packetIndex = 0
...
func (d *Decoder) iNotSync(lastByte byte) error {
	if lastByte == 0 {
		if len(d.ctx.raw) > 1 {
			d.ctx.dumpUnsyncedBytes = len(d.ctx.raw) - 1
			d.ctx.processState = stateSendUnsynced
			d.ctx.updateUnsyncPacketIndex = d.ctx.BlockBaseIdx + trace.Index(d.ctx.BytesConsumed()) - 1
		} else {
			d.ctx.packetIndex = d.ctx.BlockBaseIdx + trace.Index(d.ctx.BytesConsumed()) - 1
		}
		d.ctx.currHandler = d.ctx.table[lastByte].handler
	} else if len(d.ctx.raw) >= 8 {
		d.ctx.dumpUnsyncedBytes = len(d.ctx.raw)
		d.ctx.processState = stateSendUnsynced
		d.ctx.updateUnsyncPacketIndex = d.ctx.BlockBaseIdx + trace.Index(d.ctx.BytesConsumed())
	}
	return nil
}

func (d *Decoder) iPktNoPayload(lastByte byte) error {
...
func (d *Decoder) iPktExtension(lastByte byte) error {
	if len(d.ctx.raw) != 2 {
		return nil
	}
	...
	case 0x00:
		...
		d.ctx.currHandler = (*Decoder).iPktASync
	case 0x03:
...
func (d *Decoder) iPktASync(lastByte byte) error {
	if lastByte != 0 {
...
func (d *Decoder) iPktTraceInfo(lastByte byte) error {
	...
	if d.ctx.tinfoSectFlags == tinfoAll {
		...
		d.ctx.currPacket.TraceInfo = TraceInfo{}
...
func (d *Decoder) iPktTimestamp(lastByte byte) error {
	if len(d.ctx.raw) == 1 {
...
func (d *Decoder) iPktException(lastByte byte) error {
	switch len(d.ctx.raw) {
	case 1:
		d.ctx.excepSize = 3
	case 2:
	...
	if len(d.ctx.raw) == d.ctx.excepSize {
		...
		d.ctx.currPacket.Exception.MType = d.Config.CoreProf == trace.ProfileCortexM
		d.ctx.processState = stateSendPkt

		if d.Config.IsETE() {
			if excepType == 0x0 || excepType == 0x18 {
				d.ctx.currPacket.Addr = Address{Size: 64, ValidBits: 64}
				if excepType == 0x18 {
...
func (d *Decoder) iPktITE(lastByte byte) error {
	if len(d.ctx.raw) < 10 {
		return nil
	}
...
func (d *Decoder) iPktCycleCntF123(lastByte byte) error {
	if len(d.ctx.raw) == 1 {
		h := d.ctx.raw[0]
...
func (d *Decoder) iPktSpeclRes(lastByte byte) error {
	h := d.ctx.raw[0]
	if len(d.ctx.raw) == 1 {
		switch d.ctx.currPacket.Type {
		case PktMispredict:
			switch h & 0x3 {
			case 0x1:
				d.ctx.currPacket.Atom = Atom{EnBits: 0x1, Num: 1}
			case 0x2:
				d.ctx.currPacket.Atom = Atom{EnBits: 0x3, Num: 2}
			case 0x3:
				d.ctx.currPacket.Atom = Atom{EnBits: 0x0, Num: 1}
			}
		...
		case PktCancelF3:
			...
			d.ctx.currPacket.CancelValid = true
			if h&0x1 != 0 {
				d.ctx.currPacket.Atom = Atom{EnBits: 1, Num: 1}
			}
			d.ctx.processState = stateSendPkt
		}
		return nil
...
func (d *Decoder) iPktContext(lastByte byte) error {
	switch len(d.ctx.raw) {
	case 1:
		d.ctx.vmidBytes = 0
		d.ctx.ctxtIDBytes = 0
...
func (d *Decoder) iPktAddrCtxt(lastByte byte) error {
	if len(d.ctx.raw) == 1 {
		h := d.ctx.raw[0]
...
func (d *Decoder) iPktShortAddr(lastByte byte) error {
	if len(d.ctx.raw) == 1 {
		d.ctx.addrIS = packetAddrIS(d.ctx.currPacket.Type)
		return nil
	}
...
func (d *Decoder) iPktLongAddr(lastByte byte) error {
	if len(d.ctx.raw) == 1 {
		h := d.ctx.raw[0]
		d.ctx.addrIS = packetAddrIS(d.ctx.currPacket.Type)
...
func (d *Decoder) iPktQ(lastByte byte) error {
	if len(d.ctx.raw) == 1 {
		h := d.ctx.raw[0]
		d.ctx.qType = h & 0xF
...
func (d *Decoder) iAtom(lastByte byte) error {
	h := d.ctx.raw[0]
	var pattern uint32
	var count uint8
	switch d.ctx.currPacket.Type {
	...
	d.ctx.currPacket.Atom = Atom{EnBits: pattern, Num: count}
	d.ctx.processState = stateSendPkt
	return nil
}
...
func (d *Decoder) setLongAddr(idx int, is uint8, is64 bool) {
	var value uint64
	if is == 0 {
...
func contextPayloadBytes(info byte, cfg *Config) (vmidBytes, ctxtIDBytes int) {
	if info&0x40 != 0 {
...
func extractAndSetContextInfo(pkt *Packet, b []byte, idx int, cfg *Config) {
	if idx >= len(b) {
		return
	}
	info := b[idx]
...
func packetAddrIS(t PacketType) uint8 {
	switch t {
...
func (d *Decoder) buildPacketTable() {
	for i := range d.ctx.table {
		d.ctx.table[i] = packetTableEntry{PktReserved, (*Decoder).iPktReserved}
	}
	set := func(h byte, t PacketType, fn packetHandler) { d.ctx.table[h] = packetTableEntry{t, fn} }
	set(0x00, PktExtension, (*Decoder).iPktExtension)
	set(0x01, PktTraceInfo, (*Decoder).iPktTraceInfo)
	set(0x02, PktTimestamp, (*Decoder).iPktTimestamp)
	set(0x03, PktTimestamp, (*Decoder).iPktTimestamp)
	set(0x04, PktTraceOn, (*Decoder).iPktNoPayload)
	set(0x05, PktFuncRet, (*Decoder).iPktNoPayload)
	set(0x06, PktException, (*Decoder).iPktException)
	exceptRtnFn := (*Decoder).iPktNoPayload
	if d.Config.IsETE() {
		exceptRtnFn = (*Decoder).iPktInvalidCfg
	}
	set(0x07, PktExceptionReturn, exceptRtnFn)
	if d.Config.IsETE() {
		set(0x09, PktITE, (*Decoder).iPktITE)
		set(0x0A, PktTransStart, (*Decoder).iPktNoPayload)
		set(0x0B, PktTransCommit, (*Decoder).iPktNoPayload)
	}
	for i := range 4 {
		t := PktCycleCountF2
		if i >= 2 {
		...
		set(byte(0x0C+i), t, (*Decoder).iPktCycleCntF123)
	}
	for i := range 16 {
		set(byte(0x10+i), PktCycleCountF3, (*Decoder).iPktCycleCntF123)
	}
	for i := range 8 {
		fn := (*Decoder).iPktInvalidCfg
		if d.Config.EnabledDataTrace() {
			fn = (*Decoder).iPktNoPayload
		}
	...
	set(0x2D, PktCommit, (*Decoder).iPktSpeclRes)
	set(0x2E, PktCancelF1, (*Decoder).iPktSpeclRes)
	set(0x2F, PktCancelF1Mispred, (*Decoder).iPktSpeclRes)
	for i := range 4 {
		set(byte(0x30+i), PktMispredict, (*Decoder).iPktSpeclRes)
		set(byte(0x34+i), PktCancelF2, (*Decoder).iPktSpeclRes)
	}
	for i := range 8 {
		set(byte(0x38+i), PktCancelF3, (*Decoder).iPktSpeclRes)
	}
	condFn := (*Decoder).iPktUnsupported
	if d.Config.HasCondTrace() && d.Config.EnabledCondITrace() != CondTraceDisabled {
		condFn = (*Decoder).iPktNoPayload
	}
	...
	if d.Config.FullVersion() >= 0x43 {
		set(0x70, PktIgnore, (*Decoder).iPktNoPayload)
	}
	for i := range 15 {
		set(byte(0x71+i), PktEvent, (*Decoder).iPktNoPayload)
	}
	set(0x80, PktContext, (*Decoder).iPktContext)
	set(0x81, PktContext, (*Decoder).iPktContext)
	set(0x82, PktAddrCtxtL32IS0, (*Decoder).iPktAddrCtxt)
	set(0x83, PktAddrCtxtL32IS1, (*Decoder).iPktAddrCtxt)
	set(0x85, PktAddrCtxtL64IS0, (*Decoder).iPktAddrCtxt)
	set(0x86, PktAddrCtxtL64IS1, (*Decoder).iPktAddrCtxt)
	if d.Config.FullVersion() >= 0x46 {
		set(0x88, PktTSMarker, (*Decoder).iPktNoPayload)
	}
	for i := range 3 {
		set(byte(0x90+i), PktAddrMatch, (*Decoder).iPktNoPayload)
	}
	set(0x95, PktAddrSIS0, (*Decoder).iPktShortAddr)
	set(0x96, PktAddrSIS1, (*Decoder).iPktShortAddr)
	set(0x9A, PktAddrL32IS0, (*Decoder).iPktLongAddr)
	set(0x9B, PktAddrL32IS1, (*Decoder).iPktLongAddr)
	set(0x9D, PktAddrL64IS0, (*Decoder).iPktLongAddr)
	set(0x9E, PktAddrL64IS1, (*Decoder).iPktLongAddr)
	for i := range 16 {
		switch i {
		case 3, 4, 7, 8, 9, 13, 14:
		default:
			if d.Config.HasQElem() {
				set(byte(0xA0+i), PktQ, (*Decoder).iPktQ)
			}
		}
	}
	if d.Config.IsETE() {
		for i := range 3 {
			set(byte(0xB0+i), PktSrcAddrMatch, (*Decoder).iPktNoPayload)
		}
		set(0xB4, PktSrcAddrSIS0, (*Decoder).iPktShortAddr)
		set(0xB5, PktSrcAddrSIS1, (*Decoder).iPktShortAddr)
		set(0xB6, PktSrcAddrL32IS0, (*Decoder).iPktLongAddr)
		set(0xB7, PktSrcAddrL32IS1, (*Decoder).iPktLongAddr)
		set(0xB8, PktSrcAddrL64IS0, (*Decoder).iPktLongAddr)
		set(0xB9, PktSrcAddrL64IS1, (*Decoder).iPktLongAddr)
	}
	for i := 0xC0; i <= 0xD4; i++ {
		set(byte(i), PktAtomF6, (*Decoder).iAtom)
	}
	for i := 0xD5; i <= 0xD7; i++ {
		set(byte(i), PktAtomF5, (*Decoder).iAtom)
	}
	for i := 0xD8; i <= 0xDB; i++ {
		set(byte(i), PktAtomF2, (*Decoder).iAtom)
	}
	for i := 0xDC; i <= 0xDF; i++ {
		set(byte(i), PktAtomF4, (*Decoder).iAtom)
	}
	for i := 0xE0; i <= 0xF4; i++ {
		set(byte(i), PktAtomF6, (*Decoder).iAtom)
	}
	set(0xF5, PktAtomF5, (*Decoder).iAtom)
	set(0xF6, PktAtomF1, (*Decoder).iAtom)
	set(0xF7, PktAtomF1, (*Decoder).iAtom)
	for i := 0xF8; i <= 0xFF; i++ {
		set(byte(i), PktAtomF3, (*Decoder).iAtom)
	}
}
