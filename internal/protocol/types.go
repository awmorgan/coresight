...
type InstrInfo struct {
	...
	Type              trace.InstrType
	BranchAddr        trace.VAddr
	NextISA           trace.ISA
	InstrSize         uint8
	IsConditional     bool
	IsLink            bool
	ThumbItConditions uint8
	Subtype           trace.InstrSubtype
}

type ArchProfile struct {
	Arch    trace.ArchVersion
	Profile trace.CoreProfile
}

func IsV8Arch(arch trace.ArchVersion) bool              { return arch >= trace.ArchV8 && arch <= trace.ArchV8max }
func IsArchMinVer(arch, minArch trace.ArchVersion) bool { return arch >= minArch }
...
const (
	BadIndex   trace.Index = ^trace.Index(0)
	MaxTraceID             = 128
)
