...
func memBuffDemoConfig() *etmv4.Config {
...
const (
	...
	memDumpAddress = trace.VAddr(0xFFFFFFC000081000)
...
func runMemBuffDemo(t *testing.T, sb *strings.Builder, mapper *memacc.GlobalMapper) uint32 {
	t.Helper()
	...
	pipe, err := pipeline.NewPipeline(true, demux.DemuxOptions{
		FrameMemAlign: true,
	})
	if err != nil {
	...
	for {
		...
		if n > 0 {
			_, writeErr := pipe.Write(trace.Index(totalBytes), buf[:n])
			totalBytes += uint32(n)
			if writeErr != nil {
				break
			}
...
func TestMemBuffDemoGoldens(t *testing.T) {
	...
	t.Run("mem_buff_demo", func(t *testing.T) {
		...
		block2St := memDumpAddress + trace.VAddr(block1Sz)

		mapper := memacc.NewGlobalMapper()

		acc1 := memacc.NewBufferAccessor(block1St, kernelDump[:block1Sz], trace.MemSpaceAny, "")
		if err := mapper.AddAccessor(acc1, trace.BadCSSrcID); err != nil {
			t.Fatalf("AddAccessor block1: %v", err)
		}
		acc2 := memacc.NewBufferAccessor(block2St, kernelDump[block1Sz:block1Sz+block2Sz], trace.MemSpaceAny, "")
		if err := mapper.AddAccessor(acc2, trace.BadCSSrcID); err != nil {
	...
	t.Run("mem_buff_demo_cb", func(t *testing.T) {
		...
		mapper := memacc.NewGlobalMapper()
		endAddr := memDumpAddress + trace.VAddr(programImageSize) - 1
		cbAcc := memacc.NewCallbackAccessor(memDumpAddress, endAddr, trace.MemSpaceAny)

		// The C++ callback copies from program_image_buffer.
		// We replicate this with a simple Go closure.
		cbAcc.SetCallback(func(address trace.VAddr, _ trace.MemSpaceAcc, _ uint8, reqBytes uint32, buffer []byte) uint32 {
			bufEndAddress := memDumpAddress + trace.VAddr(programImageSize) - 1
			if address < memDumpAddress || address > bufEndAddress {
				return 0
			}
			readBytes := reqBytes
			if address+trace.VAddr(reqBytes)-1 > bufEndAddress {
		...
		if err := mapper.AddAccessor(cbAcc, trace.BadCSSrcID); err != nil {
