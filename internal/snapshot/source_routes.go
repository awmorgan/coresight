...
func (b *PipelineBuilder) sourceRouteSpecs(tree *TraceBufferSourceTree) ([]sourceRouteSpec, []error) {
	...
	for _, sourceName := range sourceNames {
		...
		if devSrc == nil {
			...
			skipped = append(skipped, fmt.Errorf("%s", msg))
			continue
		}
		...
		if coreName == "" || coreName == "<none>" {
			if protocolRequiresCore(devType) {
				skipped = append(skipped, fmt.Errorf("source %q has no associated PE core", sourceName))
				continue
			}
		} else {
	...
	for _, spec := range specs {
		...
		if !isSupported {
			if isKnownUnsupportedPEProtocol(devType) {
				skipped = append(skipped, fmt.Errorf("unsupported PE decoder protocol %q for source %q", devType, spec.sourceName))
			} else {
				skipped = append(skipped, fmt.Errorf("create PE route for %q: unknown PE device type %q", spec.sourceName, devType))
			}
			continue
		}

		if err != nil {
			skipped = append(skipped, fmt.Errorf("create PE route for %q: %w", spec.sourceName, err))
			continue
		}
