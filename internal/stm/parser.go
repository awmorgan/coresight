...
type pktFn func(*Decoder) error

type parseContext struct {
	processState processState

	data      []byte
	blockBase trace.Index
	off       int

	currPacket Packet
	packetIdx  trace.Index
	packetData []byte
...
func (d *Decoder) processData(index trace.Index, dataBlock []byte) (uint32, error) {
	...
	for d.dataToProcess() && err == nil {
		...
		case stateWaitSync:
			d.waitForSync(index)
		case stateProcHdr:
			d.ctx.packetIdx = index + trace.Index(d.ctx.off)
			if d.readNibble() {
...
func (d *Decoder) dataToProcess() bool {
	return d.ctx.off < len(d.ctx.data) || d.ctx.nibbleSecondValid || d.ctx.processState == stateSendPkt
}
...
func (d *Decoder) readNibble() bool {
	if d.ctx.nibbleSecondValid {
...
func (d *Decoder) savePacketByte(b byte) {
	d.ctx.packetData = append(d.ctx.packetData, b)
}

func (d *Decoder) checkSyncNibble() {
	if d.ctx.nibble == 0xF {
		if !d.ctx.syncStart {
			d.ctx.syncIndex = d.ctx.blockBase + trace.Index(d.ctx.off-1)
...
func (d *Decoder) waitForSync(blockStart trace.Index) {
	bSyncOnEntry := d.ctx.isSync
	d.ctx.packetIdx = blockStart + trace.Index(d.ctx.off)
	...
	if !d.ctx.isSync || bPreAmbleAndSync {
		d.ctx.currPacket.SetType(PktNotSync, false)
	} else {
...
func (d *Decoder) op1(n uint8) pktFn {
	switch n {
	case 0x0:
		return (*Decoder).stmPktNull
	case 0x1:
		return (*Decoder).stmPktM8
	case 0x2:
		return (*Decoder).stmPktMERR
	case 0x3:
		return (*Decoder).stmPktC8
	case 0x4:
		return (*Decoder).stmPktD8
	case 0x5:
		return (*Decoder).stmPktD16
	case 0x6:
		return (*Decoder).stmPktD32
	case 0x7:
		return (*Decoder).stmPktD64
	case 0x8:
		return (*Decoder).stmPktD8MTS
	case 0x9:
		return (*Decoder).stmPktD16MTS
	case 0xA:
		return (*Decoder).stmPktD32MTS
	case 0xB:
		return (*Decoder).stmPktD64MTS
	case 0xC:
		return (*Decoder).stmPktD4
	case 0xD:
		return (*Decoder).stmPktD4MTS
	case 0xE:
		return (*Decoder).stmPktFlagTS
	case 0xF:
		return (*Decoder).stmPktFExt
	default:
		return (*Decoder).stmPktReserved
	}
}

func (d *Decoder) op2(n uint8) pktFn {
	switch n {
	case 0x0:
		return (*Decoder).stmPktF0Ext
	case 0x2:
		return (*Decoder).stmPktGERR
	case 0x3:
		return (*Decoder).stmPktC16
	case 0x4:
		return (*Decoder).stmPktD8TS
	case 0x5:
		return (*Decoder).stmPktD16TS
	case 0x6:
		return (*Decoder).stmPktD32TS
	case 0x7:
		return (*Decoder).stmPktD64TS
	case 0x8:
		return (*Decoder).stmPktD8M
	case 0x9:
		return (*Decoder).stmPktD16M
	case 0xA:
		return (*Decoder).stmPktD32M
	case 0xB:
		return (*Decoder).stmPktD64M
	case 0xC:
		return (*Decoder).stmPktD4TS
	case 0xD:
		return (*Decoder).stmPktD4M
	case 0xE:
		return (*Decoder).stmPktFlag
	case 0xF:
		return (*Decoder).stmPktASync
	default:
		return (*Decoder).stmPktReservedFn
	}
}

func (d *Decoder) op3(n uint8) pktFn {
	switch n {
	case 0x0:
		return (*Decoder).stmPktVersion
	case 0x1:
		return (*Decoder).stmPktNullTS
	case 0x6:
		return (*Decoder).stmPktTrigger
	case 0x7:
		return (*Decoder).stmPktTriggerTS
	case 0x8:
		return (*Decoder).stmPktFreq
	default:
		return (*Decoder).stmPktReservedF0n
	}
}

func (d *Decoder) pktNeedsTS() {
	d.ctx.needsTS = true
