...
type cAPITracePrinter struct {
	...
	elems  printers.ElementFormatter
}

func (p cAPITracePrinter) ObservePacket(index trace.Index, pkt fmt.Stringer, rawData []byte) {
	if len(rawData) == 0 {
		return
	}
...
func (p cAPITracePrinter) PrintElement(elem trace.Element) {
...
func runTrace(t *testing.T, pipe *pipeline.Pipeline, tracePath string) uint32 {
	t.Helper()
	...
	for {
		...
		if n > 0 {
			if _, err := pipe.Write(trace.Index(total), buf[:n]); err != nil {
