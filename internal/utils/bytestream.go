...
// Reader provides an idiomatic way to read from a stream of data blocks.
...
// NewReader creates a new byte stream reader.
...
func (r *Reader) DiscardScratchPrefix(n int) {
	if n <= 0 {
		return
	}
	if n >= r.scratchLen {
...
func (r *Reader) WriteScratch(b byte) {
	r.appendScratch(b)
}
...
func (r *Reader) UnreadByte() error {
	if r.off <= 0 || r.scratchLen <= 0 {
		return errors.New("utils.Reader: invalid use of UnreadByte")
	}
	r.off--
	r.scratchLen--
...
func (r *Reader) appendScratch(b byte) {
